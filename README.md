...
| 10-cnpg-postgres.provisioners.yaml                         | `postgres`                  | (any)   | `database`, `username`, `instances`, `storage`, `imageName`            | `host`, `port`, `name`, `database`, `username`, `password` | Generates a CloudNativePG (CNPG) `Cluster` and the `Secret` holding its application credentials.
